...
func TestResultContextPool(t *testing.T) {
	...
	t.Run("panics on configuration after init", func(t *testing.T) {
		t.Run("before wait", func(t *testing.T) {
			t.Parallel()
			g := NewWithResults().WithContext(context.Background())
			g.Go(func(context.Context) (interface{}, error) { return 0, nil })
	...
	t.Run("behaves the same as ErrorGroup", func(t *testing.T) {
		...
		t.Run("wait returns no error if no errors", func(t *testing.T) {
			t.Parallel()
			g := NewWithResults().WithContext(bgctx)
			g.Go(func(context.Context) (interface{}, error) { return 0, nil })
			res, err := g.Wait()
			require.Len(t, res, 1)
		...
		t.Run("wait error if func returns error", func(t *testing.T) {
			t.Parallel()
			g := NewWithResults().WithContext(bgctx)
			g.Go(func(context.Context) (interface{}, error) { return 0, err1 })
			res, err := g.Wait()
			require.Len(t, res, 0)
		...
		t.Run("wait error is all returned errors", func(t *testing.T) {
			t.Parallel()
			g := NewWithResults().WithErrors().WithContext(bgctx)
			g.Go(func(context.Context) (interface{}, error) { return 0, err1 })
			g.Go(func(context.Context) (interface{}, error) { return 0, nil })
			g.Go(func(context.Context) (interface{}, error) { return 0, err2 })
			res, err := g.Wait()
			require.Len(t, res, 1)
			require.ErrorIs(t, err, err1)
	...
	t.Run("context cancel propagates", func(t *testing.T) {
		...
		g := NewWithResults().WithContext(ctx)
	...
	t.Run("WithCancelOnError", func(t *testing.T) {
		t.Parallel()
		g := NewWithResults().WithContext(context.Background()).WithCancelOnError()
		g.Go(func(ctx context.Context) (interface{}, error) {
			<-ctx.Done()
			return 0, ctx.Err()
		})
	...
	t.Run("WithFailFast", func(t *testing.T) {
		t.Parallel()
		p := NewWithResults().WithContext(context.Background()).WithFailFast()
		p.Go(func(ctx context.Context) (interface{}, error) {
			return 0, err1
		})
		p.Go(func(ctx context.Context) (interface{}, error) {
			<-ctx.Done()
	...
	t.Run("WithCancelOnError and panic", func(t *testing.T) {
		...
		assert.EqualValues(t, 2, atomic.LoadInt64(&cancelledTasks))
	})
	...
	t.Run("WithCollectErrored", func(t *testing.T) {
		t.Parallel()
		g := NewWithResults().WithContext(context.Background()).WithCollectErrored()
		g.Go(func(context.Context) (interface{}, error) { return 0, err1 })
		res, err := g.Wait()
	...
	t.Run("WithFirstError", func(t *testing.T) {
		t.Parallel()
		g := NewWithResults().WithContext(context.Background()).WithFirstError()
	...
	t.Run("limit", func(t *testing.T) {
		...
		for _, maxConcurrency := range []int{1, 10, 100} {
			t.Run(strconv.Itoa(maxConcurrency), func(t *testing.T) {
				...
				g := NewWithResults().WithContext(ctx).WithMaxGoroutines(maxConcurrency)
				...
				for i := 0; i < taskCount; i++ {
					i := i
					expected[i] = i
					g.Go(func(context.Context) (interface{}, error) {
