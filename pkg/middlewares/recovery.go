...
func RecoveryWithMetric(from func(context.Context) gormx.Logger, handlerMetric func(method, path string)) gin.HandlerFunc {
	return func(c *gin.Context) {
		defer func() {
			if r := recover(); r != nil {
				var brokenPipe bool
				...
				handlerMetric(c.Request.Method, c.Request.URL.String())
				server.Error(ctx, code.ErrInternalServerError.WithResult(extra))
				resp.Error(c, code.ErrInternalServerError.WithResult(extra))
			}
		}()
