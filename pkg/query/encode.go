...
func reflectValue(values url.Values, val reflect.Value, scope string) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		isEmpty := isEmptyValue(sv)
		if opts.Contains("omitempty") && isEmpty {
			continue
		}

		if isEmpty {
			dv := opts.GetValue("default")
			if dv != "" {
				values.Add(name, dv)
				continue
			}
...
// GetValue get the specified option value.
func (o tagOptions) GetValue(option string) string {
	for _, s := range o {
		if strings.HasPrefix(s, option+"=") {
			return s[len(option)+1:]
		}
	}
	return ""
}
